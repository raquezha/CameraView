...
public class Camera1Engine extends CameraBaseEngine implements
    ...
    private boolean applyExposureCorrection(@NonNull Camera.Parameters params,
        ...
        if (mCameraOptions.isExposureCorrectionSupported()) {
            ...
            val = (val < min) ? min : Math.min(val, max); // cap
    ...
    private void sortRanges(List<int[]> fpsRanges) {
        if (getPreviewFrameRateExact() && mPreviewFrameRate != 0F) { // sort by range width in ascending order
            Collections.sort(fpsRanges, (range1, range2) -> (range1[1] - range1[0]) - (range2[1] - range2[0]));
        } else { // sort by range width in descending order
            Collections.sort(fpsRanges, (range1, range2) -> (range2[1] - range2[0]) - (range1[1] - range1[0]));
        }
    }
    ...
    public void startAutoFocus(@Nullable final Gesture gesture,
        ...
        getOrchestrator().scheduleStateful("auto focus", CameraState.BIND, () -> {
                    ...
                    if (shouldResetAutoFocus()) {
                        getOrchestrator().scheduleStatefulDelayed(
                                ...
                                () -> {
                                    ...
                                    int maxAF1 = params1.getMaxNumFocusAreas();
                                    int maxAE1 = params1.getMaxNumMeteringAreas();
                                    if (maxAF1 > 0) params1.setFocusAreas(null);
                                    if (maxAE1 > 0) params1.setMeteringAreas(null);
                                    applyDefaultFocus(params1); // Revert to internal focus.
